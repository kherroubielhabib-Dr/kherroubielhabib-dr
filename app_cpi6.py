...
/* Select slider — fix RTL direction */
...
/* Score buttons — prevent text wrap on mobile */
...
# ── توليد تقرير HTML قابل للتصدير ──────────────────────────────────
def generate_report_html(session, all_sessions):
    s = session
    lvl = get_level(s["cpi"])
    date_str = s["date"]
    label = s["label"]
    ...
    # Radar SVG
    radar = radar_svg(s["scores"], size=260)

    # Gauge SVG
    gauge = gauge_svg(cpi)

    # Trend SVG
    trend = trend_svg(all_sessions) if len(all_sessions) >= 2 else ""

    # Dimensions table
    dim_rows = ""
    for d in DIMS:
        v = s["scores"].get(d["key"], 0)
        pct = (v / 4) * 100
        icon = "🔴" if v <= 2 else ("🔵" if v == 3 else "🟢")
        bar = f'<div style="height:8px;background:#F3F4F6;border-radius:4px;overflow:hidden;margin-top:4px"><div style="height:100%;width:{pct}%;background:{d["color"]};border-radius:4px"></div></div>'
        dim_rows += f'''
        <tr>
          <td style="padding:10px 14px;font-weight:700;color:{d["color"]}">{d["key"]}</td>
          <td style="padding:10px 14px;font-weight:600">{d["ar"]}</td>
          <td style="padding:10px 14px;font-family:monospace;font-weight:700;color:{d["color"]}">{v}/4 {icon}</td>
          <td style="padding:10px 14px;width:160px">{bar}</td>
        </tr>'''

    # Recommendations
    weak = [d for d in DIMS if s["scores"].get(d["key"], 0) <= 2]
    recs_html = ""
    if not weak:
        recs_html = '<p style="color:#10B981;font-weight:600">✓ جميع الأبعاد في مستوى جيد.</p>'
    else:
        for d in weak:
            key = d["key"]
    ...
    for hs in all_sessions:
        ...
        history_rows += f'''
        <tr>
          <td style="padding:8px 12px">{hs["label"]}</td>
          <td style="padding:8px 12px;color:#6B7280">{hs["date"]}</td>
          <td style="padding:8px 12px;font-weight:800;font-family:monospace;color:{hl["color"]}">{hs["cpi"]}%</td>
          <td style="padding:8px 12px;font-size:12px;color:{hl["color"]}">{hl["label"]}</td>
        </tr>'''
...
<style>
  * {{ margin:0; padding:0; box-sizing:border-box; }}
  body {{ font-family:"IBM Plex Sans Arabic","Segoe UI",sans-serif; background:#F8FAFC; color:#111827; direction:rtl; padding:24px; }}
  ...
  .meta-item {{ background:#F9FAFB; border:1px solid #E5E7EB; border-radius:8px; padding:8px 16px; }}
  .meta-label {{ font-size:10px; color:#9CA3AF; }}
  .meta-val {{ font-size:18px; font-weight:800; font-family:monospace; }}
  ...
  .viz {{ display:flex; justify-content:space-around; align-items:center; flex-wrap:wrap; gap:16px; }}
...
<div class="header">
  <div>
    <h1>🧠 تقرير مؤشر التلاقح المعرفي</h1>
    <p>Cross-Pollination Index — CPI Report · د. الحبيب خروبي · ESU-001</p>
  </div>
...
<div class="card">
  ...
  <div class="meta">
    <div class="meta-item">
      <div class="meta-label">اسم الجلسة</div>
      <div style="font-size:15px;font-weight:700">{label}</div>
    </div>
    <div class="meta-item">
      <div class="meta-label">التاريخ</div>
      <div style="font-size:15px;font-weight:700">{date_str}</div>
    </div>
    <div class="meta-item">
      <div class="meta-label">CPI</div>
      <div class="meta-val" style="color:{lvl["color"]}">{cpi}%</div>
    </div>
    <div class="meta-item">
...
<div class="card">
  ...
  <div class="viz">
    <div style="text-align:center">
      <div style="font-size:12px;color:#6B7280;margin-bottom:6px">مقياس CPI</div>
      {gauge}
    </div>
    <div>
      <div style="font-size:12px;color:#6B7280;margin-bottom:6px;text-align:center">خارطة الأبعاد</div>
      {radar}
    </div>
  </div>
</div>
...
<div class="card">
  <h2>تفصيل الأبعاد الأربعة</h2>
  <table>
    <thead><tr><th>الرمز</th><th>البُعد</th><th>الدرجة</th><th>التمثيل</th></tr></thead>
...
{"" if len(all_sessions) < 2 else f'''<div class="card">
  <h2>مسار CPI عبر الزمن</h2>
  {trend}
  <table style="margin-top:12px">
    <thead><tr><th>الجلسة</th><th>التاريخ</th><th>CPI</th><th>المستوى</th></tr></thead>
    <tbody>{history_rows}</tbody>
  </table>
</div>'''}

<div class="footer">
  CPI — مؤشر التلاقح المعرفي · د. الحبيب خروبي · ESU-001 · {datetime.datetime.now().strftime("%Y-%m-%d")}
</div>
...
with tab1:
    ...
    # أزرار التقييم — HTML أفقي حقيقي لا يتكسر على الموبايل
    for d in DIMS:
        ...
        <div style="background:white; border:1.5px solid {border_color};
            ...
            <div style="font-size:11px; color:#9CA3AF; margin-bottom:3px">{d['desc']}</div>
            <em style="font-size:12px; color:#374151">«{d['q']}»</em>
        </div>
        """, unsafe_allow_html=True)

        labels = {1: "ضعيف", 2: "مقبول", 3: "جيد", 4: "متقدم"}
        score_val = st.select_slider(
            label=d["key"],
            options=[1, 2, 3, 4],
            value=current if current > 0 else 1,
            format_func=lambda v: f"{v} — {labels[v]}",
            label_visibility="collapsed",
            key=f"slider_{d['key']}",
        )
...
with tab3:
    if not st.session_state.sessions:
        ...
        # زر تصدير كل الجلسات
        report_all_html = generate_report_html(
            st.session_state.sessions[-1],
            st.session_state.sessions
        )
        b64_all = base64.b64encode(report_all_html.encode("utf-8")).decode()
        filename_all = f"CPI_Report_All_{datetime.datetime.now().strftime('%Y%m%d')}.html"
        st.markdown(
        ...
        for s in reversed(st.session_state.sessions):
            ...
            # Generate per-session report
            session_html = generate_report_html(s, st.session_state.sessions)
            b64 = base64.b64encode(session_html.encode("utf-8")).decode()
            safe_label = s["label"].replace(" ", "_")[:30]
            filename = f"CPI_{safe_label}_{s['date'][:10]}.html"
