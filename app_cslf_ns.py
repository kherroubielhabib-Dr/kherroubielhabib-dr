...
def compute_cav_status(K, H, KLL, config):
    ...
    # ✅ NEW: Epistemic Null State Detection
    if K == 0 and KLL == 0:
        flags.append("epistemic_null_state")

    # ✅ NEW: NULL_STATE as primary status
    if K == 0 and KLL == 0:
        status = "NULL_STATE"
    elif "hazard_explosion" in flags:
...
scenario_text = st.text_area("السيناريو", height=200)
...
if st.button("🚀 Generate CAV"):
    ...
    cav_status, flags = compute_cav_status(K_final, H_final, KLL_final, CONFIG)

    # ==========================================
    # Epistemic Meaning
    # ==========================================
    epistemic_meaning_map = {
        "PASSED": "decision exists and is epistemically valid",
        "FAILED_HAZARD": "decision collapsed due to instability explosion",
        "FAILED_LEGITIMACY": "decision lost epistemic legitimacy",
        "FAILED_COHERENCE": "decision lost coherence",
        "NULL_STATE": "decision does not exist epistemically"
    }
    epistemic_meaning = epistemic_meaning_map.get(cav_status, "unknown epistemic state")
    ...
    CAV = {
        ...
        "epistemic_meaning": epistemic_meaning,
    ...
    # ✅ NEW: Visual distinction for NULL_STATE
    if cav_status == "NULL_STATE":
        st.error("🕳️ NULL STATE — No Decision Exists Epistemically")
    elif cav_status == "PASSED":
        st.success("✅ Epistemically Valid — Ready for R-AGAM")
    else:
        st.error(f"🔥 {cav_status} — CAV not admissible")
        
        if "epistemic_null_state" in flags:
            st.warning("⚠️ Epistemic Null State: No decision exists anymore.")
