...
# ════════════════════════════════════════════════════════════════════
# 2. تهيئة SESSION STATE (في أعلى الملف)
# ════════════════════════════════════════════════════════════════════
...
# ════════════════════════════════════════════════════════════════════
# 3. قاموس الترجمة المركزي (مختصر للعرض، سيتم إكماله)
# ════════════════════════════════════════════════════════════════════

TRANSLATIONS = {
    "app_title": {"ar": "مؤشر التلاقح المعرفي", "en": "Cross-Pollination Index", "fr": "Indice de Pollinisation Croisée"},
    "app_sub": {"ar": "CPI Dashboard v7.3 · د. الحبيب خروبي · ESU-001", "en": "CPI Dashboard v7.3 · Dr. Elhabib Kherroubi · ESU-001", "fr": "Tableau de bord CPI v7.3 · Dr. Elhabib Kherroubi · ESU-001"},
...
# ════════════════════════════════════════════════════════════════════
# 4. البيانات الأساسية (الأبعاد، المستويات، التوصيات)
# ════════════════════════════════════════════════════════════════════
...
LEVELS = {
    ...
    "en": [
        {"min": 0, "max": 25, "cls": "level-0", "title": "Knowledge Silo", "color": "#DC2626"},
    ...
    "fr": [
        {"min": 0, "max": 25, "cls": "level-0", "title": "Silo cognitif", "color": "#DC2626"},
...
def calc_cpi(scores):
    """حساب CPI وفق المعادلة (P+L+G+EH)/16×100"""
...
def inject_css(lang):
    ...
    .dim-card {{ background: #FAFBFF; border: 1px solid #E8EDF5; border-radius: 12px; padding: 16px; margin-bottom: 12px; }}
...
with tab1:
    ...
    with col1:
        team_name = st.text_input("اسم الفريق", value="فريق الابتكار", key="team_name_input")
    with col2:
        project_name = st.text_input("اسم المشروع/الجلسة", value="Sprint 1", key="project_name_input")
    
    col3, col4 = st.columns(2)
    with col3:
        session_num = st.number_input("رقم الجلسة", min_value=1, value=st.session_state.session_num_counter, key="session_num_input")
    with col4:
        session_date = st.date_input("التاريخ", value=datetime.date.today(), key="session_date_input")
    
    st.markdown("---")
    
    # اختيار نمط القياس
    mode_choice = st.radio("نمط القياس:", [t("mode_solo"), t("mode_pmp")], horizontal=True, key="assess_mode")
    ...
    if mode_choice == t("mode_solo"):
        ...
        if cpi_now:
            ...
            with col_g:
                st.markdown(f'<div class="gauge-wrap"><div style="font-size:11px;font-weight:600;color:#64748B;margin-bottom:6px;">{t("cpi_computed") if "cpi_computed" in TRANSLATIONS else "CPI المحسوب"}</div>{gauge_svg(cpi_now)}<div style="font-size:14px;font-weight:800;color:{level["color"]};margin-top:8px;background:{level["color"]}15;padding:6px 16px;border-radius:20px;display:inline-block">{level["title"]}</div></div>', unsafe_allow_html=True)
            with col_r:
    ...
    # ════════════════════════════════════════════════════════════
        ...
        for idx, p in enumerate(st.session_state.pmp_participants):
            ...
            if len(st.session_state.pmp_participants) > 1:
                if st.button(f"🗑️ {t('pmp_remove_participant') if 'pmp_remove_participant' in TRANSLATIONS else 'حذف'}", key=f"remove_{uid}"):
...
with tab2:
    if not st.session_state.sessions:
        ...
        if bdi:
            ...
            st.markdown(f'<div style="background:#F5F3FF; border:1px solid #DDD6FE; border-radius:14px; padding:16px; display:flex; justify-content:space-between; align-items:center; margin-bottom:16px;"><div><div style="font-weight:600;">{t("bdi_title") if "bdi_title" in TRANSLATIONS else "مؤشر اختفاء الحدود (BDI)"}</div><div style="font-size:11px; color:#9CA3AF;">{t("bdi_sub") if "bdi_sub" in TRANSLATIONS else "كلما اقترب من 100% كلما اقترب الفريق من العقل الجماعي"}</div></div><div style="font-size:34px; font-weight:800; font-family:monospace; color:{bdi_color}">{bdi}%</div></div>', unsafe_allow_html=True)
...
# ════════════════════════════════════════════════════════════════════
# 11. TAB 4: الميسر المعرفي (مختصر)
# ════════════════════════════════════════════════════════════════════

with tab4:
    st.markdown(f'<div style="background:linear-gradient(135deg,#0F172A,#1E293B); color:white; border-radius:16px; padding:20px; margin-bottom:16px;"><div style="display:flex; align-items:center; gap:12px;"><div style="font-size:28px;">🧠</div><div><div style="font-weight:800; font-size:18px;">{t("tab_facilitator")}</div><div style="font-size:11px; color:#94A3B8;">Powered by Claude AI</div></div></div><div style="font-size:13px; color:#CBD5E1; background:rgba(255,255,255,0.05); border-radius:10px; padding:10px; margin-top:12px;">{t("fac_sub") if "fac_sub" in TRANSLATIONS else "طرف ثالث محايد. يحلل لغة الفريق ويكشف التحيزات المعرفية."}</div></div>', unsafe_allow_html=True)
...
with tab5:
    st.markdown(f'<div style="background:linear-gradient(135deg,#0F172A,#1E293B); color:white; border-radius:16px; padding:24px; margin-bottom:20px;"><div style="display:flex; align-items:center; gap:12px;"><div style="font-size:28px;">📜</div><div><div style="font-size:18px; font-weight:800;">{t("charter_title") if "charter_title" in TRANSLATIONS else "ميثاق التلاقح المعرفي"}</div><div style="font-size:11px; color:#94A3B8;">الإصدار 1.1 · د. الحبيب خروبي · ESU-001 · جوان 2026</div></div></div><div style="font-size:13px; color:#CBD5E1; background:rgba(255,255,255,0.05); border-radius:10px; padding:12px; margin-top:12px;">{t("charter_preamble") if "charter_preamble" in TRANSLATIONS else "انطلاقاً من أن الابتكار الحقيقي لا ينتج عن تجميع المعارف الفردية بصورة منفصلة، بل عن التفاعل المنهجي بينها..."}</div></div>', unsafe_allow_html=True)
    
    st.markdown(f'<div style="background:#FFF7ED; border:1.5px solid #FCD34D; border-radius:14px; padding:18px; margin-bottom:22px; text-align:center;"><div style="font-size:11px; font-weight:700; color:#92400E; margin-bottom:8px;">المبدأ المحوري</div><div style="font-size:15px; font-weight:700; color:#78350F;">❝ {t("charter_axiom") if "charter_axiom" in TRANSLATIONS else "التواضع المعرفي ليس مجرد فضيلة أخلاقية، بل بنية تشغيلية لازمة لظهور الذكاء الجماعي."} ❞</div><div style="font-size:12px; font-family:monospace; color:#B45309; margin-top:10px;">CI = NK × EH</div></div>', unsafe_allow_html=True)
...
with tab6:
    ...
    c1.metric("إجمالي الجلسات", stats.get("total_sessions", 0))
    c2.metric("متوسط CPI", f"{stats.get('avg_cpi', 0)}%")
    c3.metric("إجمالي الفرق", stats.get("total_teams", 0))
    
    if teams:
        st.markdown("### 🏢 الفرق المسجّلة")
        for team in teams:
            st.markdown(f"- {team}")
    ...
    if st.button("📥 تصدير CSV", type="secondary"):
        ...
        if csv_data:
            st.download_button("تحميل CSV", data=csv_data, file_name=f"CPI_export_{datetime.datetime.now().strftime('%Y%m%d')}.csv", mime="text/csv")
...
<div style="text-align:center; margin-top:48px; padding:20px; background:linear-gradient(135deg,#0F172A,#1E293B); border-radius:16px;">
    ...
    <div style="font-size:11px; color:#64748B; font-family:monospace;">CPI Framework v7.3 · CI = NK × EH · {datetime.datetime.now().year}</div>
</div>
''', unsafe_allow_html=True)
