...
TRANSLATIONS = {
    ...
    # ── PMP — Participatory Measurement Protocol (v7) ───────────────
    "pmp_mode_label": {
        "ar": "نمط القياس:",
        "en": "Measurement Mode:",
        "fr": "Mode de mesure :",
    ...
    "pmp_mode_solo": {
        "ar": "👤 تقييم فردي سريع",
        "en": "👤 Quick Individual",
        "fr": "👤 Évaluation individuelle",
    },
    "pmp_mode_team": {
        "ar": "👥 القياس التشاركي (PMP)",
        "en": "👥 Participatory Measurement (PMP)",
        "fr": "👥 Mesure participative (PMP)",
    },
    "pmp_intro": {
        "ar": "كل عضو يُدخل تقييمه الخاص بشكل منفصل (تقييم أعمى). يحسب النظام CPI الجماعي ومؤشر الانسجام الإدراكي (CAI) آلياً.",
        "en": "Each member enters their own assessment separately (blind assessment). The system automatically computes the Collective CPI and Cognitive Alignment Index (CAI).",
        "fr": "Chaque membre saisit sa propre évaluation séparément (évaluation à l'aveugle). Le système calcule automatiquement le CPI collectif et l'indice d'alignement cognitif (CAI).",
    },
    "pmp_participant": {
        "ar": "المشارك",
        "en": "Participant",
    ...
    "pmp_remove_participant": {
        "ar": "🗑️ حذف",
        "en": "🗑️ Remove",
        "fr": "🗑️ Supprimer",
    },
    "pmp_compute": {
        "ar": "🧮 حساب النتائج الجماعية",
        "en": "🧮 Compute Collective Results",
        "fr": "🧮 Calculer les résultats collectifs",
    },
    "pmp_results_title": {
        "ar": "نتائج القياس التشاركي",
        "en": "Participatory Measurement Results",
        "fr": "Résultats de la mesure participative",
    },
    "pmp_collective_cpi": {
        "ar": "CPI الجماعي",
        "en": "Collective CPI",
    ...
    "pmp_cai_title": {
        "ar": "مؤشر الانسجام الإدراكي (CAI)",
        "en": "Cognitive Alignment Index (CAI)",
        "fr": "Indice d'alignement cognitif (CAI)",
    },
    "pmp_gap_map_title": {
        "ar": "خريطة الفجوات الإدراكية حسب البُعد",
        "en": "Cognitive Gap Map by Dimension",
        "fr": "Carte des écarts cognitifs par dimension",
    },
    "pmp_individual_table": {
        "ar": "تقييمات الأفراد",
        "en": "Individual Assessments",
        "fr": "Évaluations individuelles",
    },
    "pmp_discussion_title": {
        "ar": "سؤال للنقاش الجماعي",
        "en": "Discussion Prompt",
        "fr": "Question pour la discussion"
    },
    "pmp_min_participants": {
        "ar": "أضف على الأقل مشاركاً واحداً بالاسم لحساب النتائج.",
        "en": "Add at least one named participant to compute results.",
        "fr": "Ajoutez au moins un participant nommé pour calculer les résultats.",
    },
    "pmp_capped_note": {
        "ar": "⚠ المستوى محدود بسبب الانسجام الإدراكي المنخفض (CAI). لا يمكن بلوغ مستوى أعلى دون تقارب رؤى الفريق.",
        "en": "⚠ Level capped due to low Cognitive Alignment (CAI). A higher level requires closer team perception.",
        "fr": "⚠ Niveau limité en raison d'un faible alignement cognitif (CAI). Un niveau supérieur nécessite une perception d'équipe plus proche.",
    },
    "pmp_save_btn": {
        "ar": "✅ حفظ جلسة PMP",
        "en": "✅ Save PMP Session",
        "fr": "✅ Enregistrer la séance PMP",
    },
    "pmp_participants_count": {
        "ar": "عدد المشاركين",
        "en": "Participants",
        "fr": "Participants",
    },
...
# أدوار المشاركين في القياس التشاركي (PMP)
PMP_ROLES = {
    "ar": ["عضو", "قائد", "ميسر", "خبير"],
    "en": ["Member", "Leader", "Facilitator", "Expert"],
    "fr": ["Membre", "Leader", "Facilitateur", "Expert"],
}
...
# CAI gates required to "unlock" higher levels (PMP v7 — section 8)
# level-2 (المستوى 3) needs CAI>=50, level-3 needs CAI>=65,
# level-4 needs CAI>=80, level-5 needs CAI>=85
...
def get_level_v7(cpi, cai=None):
    """
    إصدار CAI-aware من get_level: إن لم يحقق الفريق مؤشر الانسجام المطلوب
    للمستوى الذي يقترحه CPI، يُكبَح المستوى للأدنى الذي يحققه CAI الحالي.
    يعيد (level_dict, capped: bool).
    """
    raw = get_level(cpi)
    if cai is None:
        return raw, False

    raw_idx = LEVEL_ORDER.index(raw["cls"])
    eff_idx = raw_idx
    for i in range(raw_idx, -1, -1):
        if cai >= CAI_GATES[LEVEL_ORDER[i]]:
            eff_idx = i
            break
        eff_idx = i - 1 if i - 1 >= 0 else 0

    eff_idx = max(0, eff_idx)
    if eff_idx == raw_idx:
        return raw, False
...
def metric_gauge_svg(value, color):
    """عداد عام بلون محدد (مستخدم لمؤشر الانسجام CAI)."""
    if value is None:
        return ""
    circ = math.pi * 58
...
def build_pdf_html(session, all_sessions, lang):
    ...
    # ── PMP — مؤشر الانسجام الإدراكي (CAI) إن وُجد ──
    cai_val = s.get("alignment_index")
    if cai_val is not None:
        cai_info = cpi_db.cai_interpretation(cai_val, lang)
        cai_label = {"ar":"مؤشر الانسجام (CAI)","en":"Alignment Index (CAI)","fr":"Indice d'alignement (CAI)"}[lang]
        cai_stat_card = f"""
  <div class="stat-card" style="border-color:{cai_info['color']}55">
    <div class="stat-val" style="color:{cai_info['color']}">{cai_val}%</div>
    <div class="stat-label">{cai_label}</div>
  </div>"""
    else:
        cai_stat_card = ""

    # ── PMP — جدول المشاركين إن وُجدوا ──
    participants = s.get("participants") or []
    if participants:
        p_rows = ""
        for p in participants:
            p_rows += f"""<tr>
              <td style="padding:8px 12px;font-size:12px;font-weight:600">{p.get('name','')}</td>
              <td style="padding:8px 12px;font-size:11px;color:#6B7280">{p.get('specialization','')}</td>
              <td style="padding:8px 12px;font-size:11px;color:#6B7280">{p.get('role','')}</td>
              <td style="padding:8px 12px;font-weight:800;font-family:monospace;font-size:14px;color:#2563EB">{p.get('individual_cpi','')}%</td>
            </tr>"""
        p_titles = {
            "ar": {"title":"تقييمات الأفراد","name":"الاسم","spec":"التخصص","role":"الدور","cpi":"CPI الفردي"},
            "en": {"title":"Individual Assessments","name":"Name","spec":"Specialization","role":"Role","cpi":"Individual CPI"},
            "fr": {"title":"Évaluations individuelles","name":"Nom","spec":"Spécialisation","role":"Rôle","cpi":"CPI individuel"},
        }[lang]
        participants_section = f"""
<div class="card">
  <h2>👥 {p_titles["title"]}</h2>
  <table>
    <thead>
      <tr style="background:#F8FAFC">
        <th style="padding:8px 12px;font-size:11px;color:#6B7280">{p_titles["name"]}</th>
        <th style="padding:8px 12px;font-size:11px;color:#6B7280">{p_titles["spec"]}</th>
        <th style="padding:8px 12px;font-size:11px;color:#6B7280">{p_titles["role"]}</th>
        <th style="padding:8px 12px;font-size:11px;color:#6B7280">{p_titles["cpi"]}</th>
      </tr>
    </thead>
    <tbody>{p_rows}</tbody>
  </table>
...
with tab1:
    ...
    # ── اختيار نمط القياس: فردي سريع / تشاركي PMP ──
    mode_choice = st.radio(
        t("pmp_mode_label"),
        options=["solo", "pmp"],
        format_func=lambda v: t("pmp_mode_solo") if v == "solo" else t("pmp_mode_team"),
        horizontal=True,
        key="assess_mode",
    )
    ...
    # ════════════════════════════════════════════════════════════
    #  نمط 1 — تقييم فردي سريع (النمط الكلاسيكي v6)
    # ════════════════════════════════════════════════════════════
    if mode_choice == "solo":
        for k in DIM_KEYS:
            d = DIMS_DATA[k]
        ...
        if cpi_now is not None:
            ...
            if st.button(f"{t('btn_record')} — CPI: {cpi_now}%", type="primary", use_container_width=True):
                ...
                num_   = int(st.session_state.get("session_num_input", 1) or 1)
    ...
    # ════════════════════════════════════════════════════════════
    #  نمط 2 — القياس التشاركي PMP (v7)
    # ════════════════════════════════════════════════════════════
    else:
        st.markdown(f"""
        <div style="background:linear-gradient(135deg,#F5F3FF,#EFF6FF);border:1.5px solid #DDD6FE;
        ...
        # تهيئة قائمة المشاركين
        if "pmp_participants" not in st.session_state:
            st.session_state.pmp_participants = [
                {"uid": 1, "name": "", "specialization": "", "role": PMP_ROLES[lang][0],
                 "scores": {"P": 1, "L": 1, "G": 1, "EH": 1}}
            ]
        ...
        for idx, p in enumerate(st.session_state.pmp_participants):
            uid = p["uid"]
            disp_name = p["name"].strip() or f"{t('pmp_participant')} {idx+1}"
            with st.expander(f"👤 {disp_name}", expanded=(idx == len(st.session_state.pmp_participants)-1)):
                ...
                with c3:
                    role_opts = PMP_ROLES[lang]
                    cur_role = p["role"] if p["role"] in role_opts else role_opts[0]
                    p["role"] = st.selectbox(t("pmp_role"), role_opts, index=role_opts.index(cur_role), key=f"pmp_role_{uid}")

                for k in DIM_KEYS:
                    d = DIMS_DATA[k]
                    cur_val = p["scores"].get(k, 1)
                    st.markdown(f"""
                    ...
                    new_val = st.select_slider(
                        label=f"{disp_name}_{k}",
                        options=[1, 2, 3, 4],
                        value=cur_val,
                        format_func=lambda v, sl=score_labels: f"{v} — {sl[v]}",
                        label_visibility="collapsed",
                        key=f"pmp_score_{uid}_{k}",
                    )
                    p["scores"][k] = new_val

                if len(st.session_state.pmp_participants) > 1:
                    if st.button(t("pmp_remove_participant"), key=f"pmp_remove_{uid}"):
                        st.session_state.pmp_participants = [
                            x for x in st.session_state.pmp_participants if x["uid"] != uid
                        ]
                        st.session_state.pmp_stats = None
                        st.rerun()

        col_add, col_compute = st.columns([1, 2])
        with col_add:
            if st.button(t("pmp_add_participant"), use_container_width=True):
                st.session_state.pmp_participants.append({
                    "uid": st.session_state.pmp_next_uid,
                    "name": "", "specialization": "", "role": PMP_ROLES[lang][0],
                    "scores": {"P": 1, "L": 1, "G": 1, "EH": 1},
                })
        ...
        with col_compute:
            if st.button(t("pmp_compute"), type="primary", use_container_width=True):
                ...
                if not named:
                    st.warning(t("pmp_min_participants"))
                else:
        ...
        if stats:
            ...
            # متوسطات الأبعاد للفريق (لـ cpi_sessions + الرادار)
            team_avg_scores = {}
            for k in DIM_KEYS:
                vals = [p["scores"].get(k, 1) for p in named]
                team_avg_scores[k] = sum(vals) / len(vals) if vals else 1

            col_g1, col_g2 = st.columns(2)
            with col_g1:
                st.markdown(f"""
                <div class="gauge-wrap">
                    <div style="font-size:11px;font-weight:600;color:#64748B;letter-spacing:0.5px;margin-bottom:6px;text-transform:uppercase">{t('pmp_collective_cpi')}</div>
                    {gauge_svg(int(round(collective_cpi)))}
                    <div style="font-size:14px;font-weight:800;color:{level['color']};margin-top:8px;
                                background:{level['color']}15;padding:6px 16px;border-radius:20px;display:inline-block">
                        {level['title']}
                    </div>
            ...
            with col_g2:
                st.markdown(f"""
                <div class="gauge-wrap">
                    <div style="font-size:11px;font-weight:600;color:#64748B;letter-spacing:0.5px;margin-bottom:6px;text-transform:uppercase">{t('pmp_cai_title')}</div>
                    {metric_gauge_svg(int(round(cai)), cai_info['color'])}
                    <div style="font-size:14px;font-weight:800;color:{cai_info['color']};margin-top:8px;
                                background:{cai_info['color']}15;padding:6px 16px;border-radius:20px;display:inline-block">
                        {cai_info['title']}
                    </div>
            ...
            if capped:
                ...
                <div style="background:#FFFBEB;border:1px solid #FCD34D;border-radius:10px;
                            padding:10px 16px;margin-top:12px;font-size:12px;color:#92400E">
                    {t('pmp_capped_note')}
                </div>
                """, unsafe_allow_html=True)

            # ── خريطة الفجوات حسب البُعد ──
            st.markdown(f"""
            <div class="cpi-card" style="margin-top:16px">
                <div style="font-size:14px;font-weight:800;color:#1E293B;margin-bottom:14px">
                    🗺️ {t('pmp_gap_map_title')}
                </div>
            """, unsafe_allow_html=True)
            for k in DIM_KEYS:
                d    = DIMS_DATA[k]
                ...
                <div style="margin-bottom:14px">
                    <div style="display:flex;justify-content:space-between;margin-bottom:5px">
                        <span style="font-size:13px;font-weight:700;color:{d['color']}">{d[lang]['name']} ({k})</span>
                        <span style="font-size:13px;font-weight:800;font-family:monospace;color:{gcol}">σ = {gap}</span>
                    </div>
                    <div class="prog-wrap"><div class="prog-bar" style="width:{pct}%;background:{gcol}"></div></div>
                </div>
                """, unsafe_allow_html=True)
            ...
            <div class="cpi-card">
                <div style="font-size:14px;font-weight:800;color:#1E293B;margin-bottom:14px">
                    👥 {t('pmp_individual_table')} ({len(named)})
                </div>
            """, unsafe_allow_html=True)
            for p, icpi in zip(named, individual_cpis):
                ilevel = get_level(icpi)
                st.markdown(f"""
                <div style="display:flex;justify-content:space-between;align-items:center;
                    ...
                    <div>
                        ...
                        <span style="font-size:11px;color:#94A3B8;margin-{'right' if lang=='ar' else 'left'}:8px">
                            {p['specialization']} · {p['role']}
                        </span>
                    </div>
                    <span style="font-weight:800;font-family:monospace;font-size:16px;color:{ilevel['color']}">{icpi}%</span>
                </div>
                """, unsafe_allow_html=True)
            st.markdown("</div>", unsafe_allow_html=True)
            ...
            st.markdown(f"""
            <div style="background:linear-gradient(135deg,#FFF7ED,#FFFBEB);border:1.5px solid #FCD34D;
                        border-radius:12px;padding:16px 20px;margin-top:8px;text-align:center">
                <div style="font-size:11px;font-weight:700;color:#92400E;letter-spacing:1px;
                            text-transform:uppercase;margin-bottom:8px">{t('pmp_discussion_title')}</div>
            ...
            if st.button(t("pmp_save_btn"), type="primary", use_container_width=True):
                ...
                for p, icpi in zip(named, individual_cpis):
                    participants_payload.append({
                        ...
                        "role": p["role"], "scores": p["scores"], "individual_cpi": icpi,
                    })
                ...
                entry = {
                    ...
                    "label": proj_,
                    "date": date_,
                    "scores": {k: round(team_avg_scores[k]) for k in DIM_KEYS},
                    "cpi": collective_cpi,
                    "lang": lang,
                    "team": team_,
                    "alignment_index": cai,
                ...
                try:
                    ...
                    sid = cpi_db.save_cpi_session({
                        ...
                        "session_number": num_,
                        "session_date": date_,
                        "score_eh": team_avg_scores["EH"],
                        "score_l":  team_avg_scores["L"],
                        "score_p":  team_avg_scores["P"],
                        "score_g":  team_avg_scores["G"],
                        "cpi_score_final": collective_cpi,
                        "alignment_index": cai,
