    ...
    def evaluate(self, pillars: dict) -> dict:
        ...
        if collapsed:
            return {
                ...
                "trace": {
                    "hash": hashlib.sha256("NULL_STATE".encode()).hexdigest()[:48],
...
if run:
    ...
    # Display details (for architect only, not for R-AGAM) - FIXED with safety checks
    with st.expander("🔬 Epistemic Details (for Architect)"):
        ...
        negation_alerts = details.get("negation_alerts", [])
        if negation_alerts:
            st.error("Negation Detected:")
            for alert in negation_alerts:
                st.write(f"  • {alert}")
        
        paradox_alerts = details.get("paradox_alerts", [])
        if paradox_alerts:
            st.warning("Value-Content Paradoxes:")
            for paradox in paradox_alerts:
                st.write(f"  • {paradox}")
        
        collapsed = details.get("collapsed", [])
        if collapsed:
            st.error(f"💥 Collapsed pillars: {collapsed}")
    ...
    <div class="forensic-box">
        <div><strong>🔑 Decision:</strong> {result.get('existence_decision', 'UNKNOWN')}</div>
        <div><strong>📊 Ontological Status:</strong> {result.get('ontological_status', 'UNKNOWN')}</div>
        <div><strong>📈 KLL:</strong> {result.get('kll', 0.0)}</div>
        <div><strong>🕐 Timestamp:</strong> {trace_data.get('timestamp', datetime.now().isoformat())[:19]}</div>
        <div><strong>🛡️ Forensic Seal:</strong><br><code>{trace.get('hash', 'N/A')}</code></div>
    </div>
    """, unsafe_allow_html=True)
    ...
    cav_data = {
        "artifact_type": "CAV",
        "version": "14.9-S",
        "decision": result.get("existence_decision", "UNKNOWN"),
        "ontological_status": result.get("ontological_status", "UNKNOWN"),
        "kll": result.get("kll", 0.0),
        "forensic_seal": trace.get("hash", "N/A"),
