# ═══════════════════════════════════════════════════════════════════
# مؤشر التلاقح المعرفي - CPI Dashboard v7.3.3
# Cross-Pollination Index · Indice de Pollinisation Croisée
# د. الحبيب خروبي · ESU-001
# الإصدار: v7.3.3 - التصحيح النهائي الكامل
# ═══════════════════════════════════════════════════════════════════
...
st.set_page_config(
    page_title="CPI Dashboard v7.3.3",
    page_icon="🧠",
    layout="wide",
...
# ════════════════════════════════════════════════════════════════════
# 3. قاموس الترجمة المركزي الكامل (v7.3.3)
# ════════════════════════════════════════════════════════════════════

TRANSLATIONS = {
    # التطبيق
    "app_title": {"ar": "مؤشر التلاقح المعرفي", "en": "Cross-Pollination Index", "fr": "Indice de Pollinisation Croisée"},
    "app_sub": {"ar": "CPI Dashboard v7.3.3 · د. الحبيب خروبي · ESU-001", "en": "CPI Dashboard v7.3.3 · Dr. Elhabib Kherroubi · ESU-001", "fr": "Tableau de bord CPI v7.3.3 · Dr. Elhabib Kherroubi · ESU-001"},
    ...
    "pmp_warning_name": {"ar": "أضف اسم مشارك واحد على الأقل", "en": "Add at least one participant name", "fr": "Ajoutez au moins un nom de participant"},
    ...
    "not_defined": {"ar": "غير محدد", "en": "Not defined", "fr": "Non défini"},
    
    # تبويب الميسر المعرفي
    ...
    "fac_err_api": {"ar": "أدخل مفتاح Anthropic API أولاً", "en": "Enter Anthropic API key first", "fr": "Entrez d'abord la clé API Anthropic"},
    "fac_err_msg": {"ar": "اكتب رسالتك أولاً", "en": "Type your message first", "fr": "Tapez d'abord votre message"},
    "fac_spinner": {"ar": "🧠 الميسر يحلل...", "en": "🧠 Facilitator is analyzing...", "fr": "🧠 Le facilitateur analyse..."},
    "fac_placeholder": {"ar": "الصق محضر الاجتماع أو اكتب سؤالك هنا...", "en": "Paste meeting minutes or type your question here...", "fr": "Collez les minutes de la réunion ou tapez votre question ici..."},
    "fac_input_lbl": {"ar": "المدخلات:", "en": "Input:", "fr": "Entrée :"},
    "fac_send_btn": {"ar": "📤 إرسال للميسر", "en": "📤 Send to Facilitator", "fr": "📤 Envoyer au facilitateur"},
    ...
    "charter_name_plc": {"ar": "ادخل اسمك...", "en": "Enter your name...", "fr": "Entrez votre nom..."},
    "charter_sign_btn": {"ar": "✍️ أوقّع", "en": "✍️ Sign", "fr": "✍️ Signer"},
    "charter_success": {"ar": "التزامك مُسجَّل --- يمكنك الانتقال إلى تقييم الجلسة.", "en": "Commitment recorded --- you may proceed to session assessment.", "fr": "Engagement enregistré --- vous pouvez passer à l'évaluation de la session."},
...
# ════════════════════════════════════════════════════════════════════
# 4. البيانات الأساسية (مع تصحيح الإملاء)
# ════════════════════════════════════════════════════════════════════
...
DIMS_DATA = {
    ...
    "EH": {"color": "#DC2626", "ar": {"name": "التواضع المعرفي", "desc": "قدرة الأفراد على قبول التصحيح والتعلم من الآخرين", "q": "هل استمعنا لبعضنا بتواضع وغيرنا مواقفنا؟"},  # تم تصحيح "موافقنا" إلى "مواقفنا"
    ...
    if not st.session_state.sessions:
        ...
        for s in reversed(st.session_state.sessions):
            ...
            with st.expander(f"📌 {s['label']} --- {s['date']} (CPI: {s['cpi']}%)"):
                ...
                with col_a:
                    st.markdown(f"**{t('hist_team')}** {s['team']}")
                    st.markdown(f"**{t('hist_level')}** {lvl['title']}")
                    st.markdown(f"**CAI:** {s.get('alignment_index', t('not_defined'))}%")
                with col_b:
...
with tab4:
    ...
    user_input = st.text_area(t("fac_input_lbl"), placeholder=t("fac_placeholder"), height=150)
    if st.button(t("fac_send_btn"), type="primary"):
        if not st.session_state.api_key:
            st.error(t("fac_err_api"))
        elif not user_input.strip():
            st.warning(t("fac_err_msg"))
        else:
            with st.spinner(t("fac_spinner")):
...
# ════════════════════════════════════════════════════════════════════
# 12. TAB 5: الميثاق (مع التصحيح)
# ════════════════════════════════════════════════════════════════════

with tab5:
    st.markdown(f'<div style="background:linear-gradient(135deg,#0F172A,#1E293B); color:white; border-radius:16px; padding:24px; margin-bottom:20px;"><div style="display:flex; align-items:center; gap:12px;"><div style="font-size:28px;">📜</div><div><div style="font-size:18px; font-weight:800;">{t("charter_title")}</div><div style="font-size:11px; color:#94A3B8;">{t("charter_version")}</div></div></div><div style="font-size:13px; color:#CBD5E1; background:rgba(255,255,255,0.05); border-radius:10px; padding:12px; margin-top:12px;">{t("charter_preamble")}</div></div>', unsafe_allow_html=True)
    
    st.markdown(f'<div style="background:#FFF7ED; border:1.5px solid #FCD34D; border-radius:14px; padding:18px; margin-bottom:22px; text-align:center;"><div style="font-size:11px; font-weight:700; color:#92400E; margin-bottom:8px;">{t("core_axiom")}</div><div style="font-size:15px; font-weight:700; color:#78350F;">❝ {t("charter_axiom")} ❞</div><div style="font-size:12px; font-family:monospace; color:#B45309; margin-top:10px;">CI = NK × EH</div></div>', unsafe_allow_html=True)
    
    signer = st.text_input(t("charter_name_lbl"), placeholder=t("charter_name_plc"))
    if st.button(t("charter_sign_btn"), type="primary"):
        if signer.strip():
            st.session_state.signatories.append({"name": signer.strip(), "time": datetime.datetime.now().strftime("%H:%M")})
            st.success(t("charter_success"))
            st.rerun()
...
# ════════════════════════════════════════════════════════════════════
# 13. TAB 6: الإدارة (مع إصلاح عرض %None)
# ════════════════════════════════════════════════════════════════════
...
with tab6:
    ...
    # إصلاح عرض %None - إذا كان avg_cpi None أو 0 يعرض 0%
    avg_val = stats.get("avg_cpi", 0)
    if avg_val is None:
        avg_val = 0
    c2.metric(t("avg_cpi"), f"{avg_val}%")
...
<div style="text-align:center; margin-top:48px; padding:20px; background:linear-gradient(135deg,#0F172A,#1E293B); border-radius:16px;">
    ...
    <div style="font-size:11px; color:#64748B; font-family:monospace;">CPI Framework v7.3.3 · CI = NK × EH · {datetime.datetime.now().year}</div>
</div>
''', unsafe_allow_html=True)
